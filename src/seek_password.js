...
function flower_password(pwd, key) {
	...
	var md5two = md5(md5one, "snow_test");
	var md5three = md5(md5one, "kise_test");
	// to uppercase
	...
	for (var i = 0; i < source.length; ++i) {
		if (isNaN(source[i])) {
			var str = "sunlovesnow1992063189xykab";
...
function seek_password(hash) {
	...
	for (var i = 0; i <= hash.length - 14; ++i) {
		var sub_hash = hash.slice(i, i + 14).split("");
		var count = 0;
